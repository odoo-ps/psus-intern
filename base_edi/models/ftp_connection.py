...
class FTPConnection(object):
    ...
    def _connect(self):
        try:
            self._conn = ftplib.FTP(host=self._host, user=self._login, passwd=self._password, timeout=10)
            if self._active:
    ...
    def upload_outgoing_file(self, outgoing_file, directory=None):
        ...
        self.upload_file(filename, content=StringIO(content_type == "xml" and content.encode("utf-8", "replace") or encodebytes(content)), directory=directory)
